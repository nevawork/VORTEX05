...
class AdvancedTicketSystem (commands .Cog ):
    ...
    async def log_ticket_event (self ,guild_id :int ,event_type :str ,data :Dict [str ,Any ]):
        ...
        try :
            ...
            if event_type =="Created":
                ...
                "╭━━━━━━━━━━━━━━━━━━━━╮\n"
                "┃ 🆕 Created\n"
                f"┃ [user] User: {creator.display_name}\n"
                f"┃ 🆔 ID: {creator.id}\n"
    ...
    @ticket .command (name ="reset",description ="[warning] Reset the entire ticket system")
    @commands .has_permissions (manage_guild =True )
    ...
    @ticket .command (name ="close",description ="[x] Close the current ticket")
    ...
    async def add (self ,ctx :commands .Context ,user :discord .Member ):
        """Add user to ticket"""
        if not await is_ticket_channel (self .bot ,ctx .channel ):
            await ctx .send ("[x] This command can only be used in ticket channels.")
            return 

        if not await self .can_manage_ticket (ctx .author ,ctx .channel ):
            await ctx .send ("[x] You don't have permission to add users to this ticket.")
            return 
    ...
    async def remove (self ,ctx :commands .Context ,user :discord .Member ):
        ...
        if not await self .can_manage_ticket (ctx .author ,ctx .channel ):
            await ctx .send ("[x] You don't have permission to remove users from this ticket.")
            return 
    ...
    async def rename (self ,ctx :commands .Context ,*,new_name :str ):
        ...
        if not await self .can_manage_ticket (ctx .author ,ctx .channel ):
            await ctx .send ("[x] You don't have permission to rename this ticket.")
            return 
    ...
    async def claim (self ,ctx :commands .Context ):
        ...
        if not await user_has_support_role (self .bot ,ctx .author ):
            await ctx .send ("[x] You don't have permission to claim tickets.")
            return 
    ...
    async def create_ticket_channel (self ,interaction :discord .Interaction ,category :str ,reason :str ):
        ...
        try :
            ...
            if not await self .check_cooldown (interaction .user .id ,interaction .guild .id ):
                ...
                "[clock] You're on cooldown! Please wait before creating another ticket.",
                ephemeral =True 
                )
                return 
...
class TicketSetupView (discord .ui .View ):
    ...
    async def finish_setup (self ,interaction :discord .Interaction ):
        ...
        try :
            ...
            f"**[users] Support Role:** {support_role.mention}\n"
            f"**[chart] Log Channel:** {log_channel.mention if log_channel else 'None'}\n"
...
class TicketAuthorInfoView (discord .ui .View ):
    ...
    async def show_author_info (self ,interaction :discord .Interaction ,button :discord .ui .Button ):
        try :
            ...
            embed .add_field (name ="[user] Display Name",value =user .display_name ,inline =True )
