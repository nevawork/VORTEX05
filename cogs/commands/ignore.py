...
class Ignore(commands.Cog):
  ...
  async def send_message(self, ctx, title=None, description=None):
    ...
    await ctx.reply(view=view, mention_Developer: NEVA "3pa8"=False)
  ...
  async def channel_add(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        await self.send_message(ctx, title="<:VORTEXINF_notify:1227866804630720565> Access Denied", description="You can only add up to 30 channels to the ignore list.")
        return
  ...
  async def user_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        await self.send_message(ctx, title="<:VORTEXINF_notify:1227866804630720565> Access Denied", description="You can only add up to 30 users to the ignore list.")
        return
