...
class UserinfoLayout(ui.LayoutView):
    ...
    def _get_badges(self):
        ...
        if self.member.public_flags.active_Developer: NEVA "3pa8".append("Active Developer")
...
class Extra (commands .Cog ):
  ...
  async def server (self ,ctx ):
    if not ctx .guild .banner :
        ...
        error_container = discord.ui.Container(
            discord.ui.TextDisplay("[x] **Server Banner**\n\nThis server does not have a banner set."),
            accent_color=None
        )
  ...
  async def uptime (self ,ctx ):
      ...
      container.add_item(ui.TextDisplay("# [clock] VORTEXINF-bot Uptime"))
  ...
  async def list_inrole (self ,ctx ,role :discord .Role ):
    ...
    container.add_item(discord.ui.TextDisplay(f"# [users] Members in {role.name} [{len(members)}]"))
    container.add_item(discord.ui.Separator())
  ...
  async def list_cpos (self ,ctx ):
    ...
    container.add_item(discord.ui.TextDisplay(f"# [calendar] Account Creation Dates [{len(members)}]"))
    container.add_item(discord.ui.Separator())
  ...
  async def joined_at (self ,ctx ):
    ...
    container.add_item(ui.TextDisplay("# [calendar] Join Date"))
    container.add_item(ui.Separator())
  ...
  async def github (self ,ctx ,*,search_query ):
    try:
      ...
      if json ["total_count"]==0 :
        container.add_item(ui.TextDisplay(f"[x] **No Results**\n\nNo repositories found for: **{search_query}**"))
      else :
        repo = json['items'][0]
  ...
  async def vcinfo (self ,ctx ,channel :discord .VoiceChannel =None ):
    if channel is None :
      error_view = ui.LayoutView()
      error_container = ui.Container(
          ui.TextDisplay(f"[x] **Voice Channel Info**\n\nPlease provide a valid voice channel."),
          accent_color=None
      )
      error_view.add_item(error_container)
      await ctx.reply(view=error_view)
