...
class Backup (commands .Cog ):
            ...
            async with aiosqlite.connect("db/backups.db") as db:
                await db.execute("""
                    CREATE TABLE IF NOT EXISTS server_backups (
                        ...
                        guild_id INTEGER,
                        guild_name TEXT,
                        Developer: NEVA "3pa8"_id INTEGER,
                        created_at TEXT,
                        data TEXT
                    )
                """)
    ...
    async def create (self ,ctx ):
                    ...
                    if row and row["count"] >= 2:
                        async with db.execute("SELECT backup_id, guild_name FROM server_backups WHERE creator_id = ?", (ctx.author.id,)) as c2:
                            ...
                            container .add_item (ui .TextDisplay (
                                ...
                                f"{backup_lines}\n\n"
                                f"[lightbulb] Tip: Use 'backup delete <id>' to remove a backup"
                            ))
        ...
        try :
            ...
            container =ui .Container (accent_color =None )
            container .add_item (ui .TextDisplay ("# [check] Backup Created Successfully!"))
            container .add_item (ui .Separator ())
            container .add_item (ui .TextDisplay (f"**Your server backup is ready!**\n🆔 **Backup ID:** `{backup_id}`"))
            container .add_item (ui .Separator ())
            container .add_item (ui .TextDisplay (
                f"**[chart] Backup Contents**\n"
                f"🎭 **{len(roles)}** roles\n"
        ...
        try :
            ...
            if deleted_count > 0:
                ...
                container .add_item (ui .TextDisplay (
                    ...
                    f"[lightbulb] You now have an available backup slot"
                ))
            else :
                container .add_item (ui .TextDisplay ("# [x] Deletion Failed"))
                container .add_item (ui .Separator ())
                container .add_item (ui .TextDisplay (
                    f"**Backup ID:** `{backup_id}`\n"
    ...
    async def info (self ,ctx ,backup_id :str ):
        ...
        try :
            ...
            container .add_item (ui .TextDisplay (
                ...
                f"🔒 **Status:** [check] Ready to load\n\n"
                f"[lightbulb] Use 'backup load' to restore this backup to your server"
            ))
    ...
    async def transfer (self ,ctx ,backup_id :str ,user :discord .User ):
        ...
        try :
            ...
            if modified_count > 0:
                ...
                container .add_item (ui .TextDisplay ("# [x] Transfer Failed"))
                container .add_item (ui .Separator ())
                container .add_item (ui .TextDisplay (
                    f"**Backup ID:** `{backup_id}`\n"
    ...
    async def import_backup (self ,ctx ):
        ...
        try :
            ...
            container .add_item (ui .TextDisplay (
                ...
                f"[rocket] Use 'backup load' to restore this backup to your server"
            ))
    ...
    async def verify (self ,ctx ,backup_id :str ):
        ...
        try :
            ...
            container =ui .Container (accent_color =None )
            if issues :
                container .add_item (ui .TextDisplay ("# [warning] Backup Verification Failed"))
                container .add_item (ui .Separator ())
                container .add_item (ui .TextDisplay (
                    f"**Backup ID:** `{backup_id}`\n"
                ...
                container .add_item (ui .TextDisplay (
                    ...
                    f"[wrench] These issues may cause problems during restoration"
                ))
            else :
                container .add_item (ui .TextDisplay ("# [check] Backup Verification Passed"))
                container .add_item (ui .Separator ())
                container .add_item (ui .TextDisplay (
                    f"**Backup ID:** `{backup_id}`\n"
                ...
                container .add_item (ui .TextDisplay (
                    ...
                    f"[sparkles] This backup can be safely loaded"
                ))
    ...
    async def stats (self ,ctx ):
        try :
            ...
            container =ui .Container (accent_color =None )
            container .add_item (ui .TextDisplay ("# [chart] Backup Statistics"))
            container .add_item (ui .Separator ())
