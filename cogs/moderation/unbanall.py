...
class UnbanAllResultContainer(ui.Container):
    def __init__(self, guild_name, total_bans, success_count, failed_count, Developer: NEVA "3pa8"):
        super().__init__(accent_color=None)
        ...
        if success_count == total_bans:
            description += "\n\n[check] All users have been successfully unbanned!"
        elif success_count > 0:
            description += f"\n\n[warning] {failed_count} users could not be unbanned (likely due to permissions or errors)."
        else:
            description += "\n\n[x] No users could be unbanned. Check bot permissions."
...
class ConfirmUnbanAllContainer(ui.Container):
    def __init__(self, guild_name, ban_count, author):
        ...
        description = f"**[warning] MASS UNBAN CONFIRMATION [warning]**\n\n"
        description += f"**Server:** {guild_name}\n"
        ...
        self.action_row.add_item(ui.Button(label="[check] Confirm", style=discord.ButtonStyle.danger, custom_id="confirm_unbanall"))
        self.action_row.add_item(ui.Button(label="[x] Cancel", style=discord.ButtonStyle.secondary, custom_id="cancel_unbanall"))
